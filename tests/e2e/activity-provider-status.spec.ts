...
test("Activity shows live campaign provider availability", async ({ page }) => {
  ...
  try {
    ...
    // delivery/attempt rows, but ProviderEvent intentionally does not cascade
    // from ProviderConnection, so remove those observations before deleting
    // the user and its provider owner rows.
