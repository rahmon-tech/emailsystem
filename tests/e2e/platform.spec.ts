...
test("login, provider setup, HTML import, preview, test, campaign controls, recovery and export", async ({
  ...
  for (const [provider, credential] of [
    ...
    await expect(
      dialog.getByText(/EmailBlast will create its permanent webhook URL automatically/),
    ).toBeVisible();
    await expect(
      input("Webhook URL (provided by EmailSystem)"),
    ).toHaveCount(0);
    await expect(
  ...
  await expect(
    page.getByText(/EmailBlast will create its permanent webhook URL automatically/),
  ).toBeVisible();
  await page
    .getByRole("button", { name: "Advanced sending speed" })
    .click();
