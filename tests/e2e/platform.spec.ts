...
test("login, provider setup, HTML import, preview, test, campaign controls, recovery and export", async ({
        ...
        : provider === "SendGrid"
          ...
          : provider === "Resend"
            ...
            : provider === "Mailgun"
              ...
              : "Callback secret (generated by EmailSystem)";
    await expect(input(deliveryCredential)).toBeVisible();
    await expect(
      dialog.getByText(/EmailSystem will create its permanent webhook URL automatically/),
    ).toBeVisible();
    await expect(
  ...
  await expect(
    page.getByText(/EmailSystem will create its permanent webhook URL automatically/),
  ).toBeVisible();
  await page
    .getByRole("button", { name: "Advanced sending speed" })
    .click();
  ...
  await expect(
    savedProviderDialog.getByLabel("Webhook URL (provided by EmailSystem)"),
  ).toHaveValue(/\/api\/webhooks\//);
  await expect(
    savedProviderDialog.getByLabel("Callback secret (generated by EmailSystem)"),
  ).toBeVisible();
  await expect(
