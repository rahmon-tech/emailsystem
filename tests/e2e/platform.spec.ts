...
test("login, provider setup, HTML import, preview, test, campaign controls, recovery and export", async ({
        ...
        : provider === "SendGrid"
          ...
          : provider === "Resend"
            ...
            : provider === "Mailgun"
              ...
              : "Callback secret (generated by EmailBlast)";
    await expect(input(deliveryCredential)).toBeVisible();
    const draftWebhookUrl = dialog.locator("code").filter({
      hasText: /\/emailblast\/api\/webhooks\/[0-9a-f-]{36}$/i,
    });
    await expect(draftWebhookUrl).toBeVisible();
    await expect(
      dialog.getByText(/permanent webhook URL for this new .* connection/i),
    ).toBeVisible();
    await expect(
  ...
  const customCallbackSecret = page.getByLabel(
    "Callback secret (generated by EmailBlast)",
  );
  await expect(customCallbackSecret).toBeVisible();
  ...
  const pendingWebhookUrl = page.locator("code").filter({
    hasText: /\/emailblast\/api\/webhooks\/[0-9a-f-]{36}$/i,
  });
  ...
  await expect(page.getByText("Delivery webhook", { exact: true })).toBeVisible();
  await page
    .getByRole("button", { name: "Save & check connection", exact: true })
    .click();
  ...
  const savedWebhookUrl = savedProviderDialog.locator("code").filter({
    hasText: /\/api\/webhooks\/[0-9a-f-]{36}$/i,
  });
  await expect(savedWebhookUrl).toBeVisible();
  await expect(savedWebhookUrl).toHaveText(
    /\/emailblast\/api\/webhooks\/[0-9a-f-]{36}$/i,
  );
  await expect(savedWebhookUrl).toHaveText(pendingWebhookText);
  const copyWebhookButton = savedProviderDialog.getByRole("button", {
    name: "Copy",
    exact: true,
  });
  await expect(copyWebhookButton).toBeVisible();
  await copyWebhookButton.click();
  await expect(
    savedProviderDialog.getByRole("button", { name: "Copied", exact: true }),
  ).toBeVisible();
  const savedCallbackSecret = savedProviderDialog.getByLabel(
    "Callback secret (generated by EmailBlast)",
  );
