...
# HTML fidelity and optional branded tracking
...
Tracking uses native PostgreSQL records and the existing web/worker stack. Each account can register up to 20 distinct owned hostnames. A campaign explicitly selects one verified hostname, or uses an administrator-selected default. There is no automatic domain rotation or reputation failover. Tracking defaults off, and an expired default falls back to direct sending; an explicit invalid campaign selection asks the user to choose a verified hostname or turn tracking off.

`TrackingLink` stores a random 256-bit opaque token, immutable destination, campaign, domain and owner. Composite foreign keys enforce owner equality. Repeated campaign submissions reuse the stored snapshot and tokens. The public redirect requires the token's assigned hostname and never accepts a destination from request parameters. It refuses expired/disabled links or locally denied destinations. The feature adds no shortener service and no browser fingerprinting.
