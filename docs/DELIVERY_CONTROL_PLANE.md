...
### Published final Activity/evidence stack

Published `main` is now exactly `5ea40e51cbbc7848b02c3945d064e0c7e86ce580`.

The formerly stacked #78 → #79 → #82 → #83 → #84 checkpoints were published by non-forced fast-forward from `ce87caa214f60247cb06a0089dcd405ed719b855`, preserving the already-verified ahead-only ancestry.
...
- PR #79 exact head `0cd6e8ac24901cff948e88015d41b0fcc3a993ef` / Quality #320: tenant-safe Activity evidence review;
...
- PR #84 exact head `5ea40e51cbbc7848b02c3945d064e0c7e86ce580` / Quality #344: final pre-publication canonical documentation reconciliation.

The same exact PR #84 head is the published `main` SHA. Push-triggered Quality #345 / run `35284283698` passed the complete assembled-main pipeline, including production-container validation.

### Current intentionally metadata-only / deferred semantics

- `text`, `hosted-image`, `attachment-only`, and `image-dominant` remain metadata-only. Repository truth does not provide a distinct deterministic runtime owner for them without redesign, so they are not development-completion blockers and must not be reported as effective behavior;
- additional effective experiment values may be recorded only where runtime proof exists; the run-start snapshot proves the approved envelope rather than metadata-only application.

### Development status

Repository development/publication acceptance is complete at exact published `main` SHA `5ea40e51cbbc7848b02c3945d064e0c7e86ce580` with Quality #345 green.

No further content mode should be bound unless repository truth proves a deterministic existing owner without redesign. Further feature changes should be driven by a concrete product requirement or defect rather than the old pre-publication checklist.

### Next phase — environment-specific acceptance

The remaining major work is outside repository-only proof:

- reconcile the actual self-hosted web + worker + PostgreSQL + Redis environment;
- verify production secrets, provider credentials, sender/domain authorization and provider-account limits;
- verify PostgreSQL/Redis persistence, migration/restore behavior and worker/web supervision;
- verify reverse proxy, TLS, base path, health/readiness and authenticated webhook reachability;
- verify observability and backup/restore;
- perform a tightly controlled legitimate production smoke test.

Vercel is not the acceptance target for the complete EmailSystem runtime. Provider-account and VPS claims must not be inferred from CI.
