...
The redirect accepts only server-stored HTTP(S) destinations on the configured `APP_URL` host and exact base-path route. Expired or locally blocked links fail closed; a query-string URL cannot override the destination. No destination fetching occurs in HTML normalization or the local reputation source. Account APIs retain existing session, CSRF, mutation-rate and owner checks.
