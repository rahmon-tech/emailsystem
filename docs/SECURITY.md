...
The redirect accepts only server-stored HTTP(S) destinations on the configured `APP_URL` host and exact matched redirect route. The public URL retains the configured base path; the handler also recognizes Next.js's already-matched internal route representation. The application trusts `X-Forwarded-Host` only behind the private application port, so the edge proxy must overwrite it. Expired or locally blocked links fail closed; a query-string URL cannot override the destination. No destination fetching occurs in HTML normalization or the local reputation source. Account APIs retain existing session, CSRF, mutation-rate and owner checks.
